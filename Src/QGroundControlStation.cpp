...
QGroundControlStation::QGroundControlStation(QObject *parent)
    : QObject(parent), d_ptr(new QGroundControlStationPrivate) {}
...
void QGroundControlStation::Init() {
    ...
    // 传递 this 指针确保线程安全
    d_ptr->setupRawBytesToBeSentCallback(
        [this](const QByteArray &data) { this->sendDataToAllLinks(data); }, this);
}
...
bool QGroundControlStation::AddDataLink(QDataLink *pDataLink) {
    ...
    /// 如果连接数太大则返回
    if(m_listLink.size() >= d_ptr->getMaxChannel()){
        return(false);
    }
    ...
    pDataLink->setIndex(m_listLink.size());

    // 连接信号槽，处理接收到的消息
    QObject::connect(
        pDataLink, &QDataLink::messageReceived, this,
        [this](const QByteArray &data) { this->processDataLinkMessage(data); });
    ...
    // 启动线程
    pThread->start();
    QMetaObject::invokeMethod(pDataLink, "connectLink", Qt::QueuedConnection);
...
void QGroundControlStation::processDataLinkMessage(const QByteArray &data) {
    QDataLink *link = qobject_cast<QDataLink *>(sender());
    if (link)
        link->feedReceivedData(data);
    if (d_ptr) {
...
void QGroundControlStation::sendDataToAllLinks(const QByteArray &data) {
    ...
    for (auto itor = m_mapLink.begin(); itor != m_mapLink.end(); ++itor) {
        ...
        if (pDataLink->isLinkConnected()) {
            // 使用 QMetaObject::invokeMethod 在目标线程中调用 onSendDataRequested
            // 槽函数 使用 Qt::QueuedConnection 确保异步执行，不阻塞
            QMetaObject::invokeMethod(pDataLink, "onSendDataRequested",

...
static spdlog::level::level_enum levelFromString(const QString &levelStr)
{
    ...
    if (l == "err" || l == "error") return spdlog::level::err;
    if (l == "critical" || l == "crit") return spdlog::level::critical;
...
void QGCSConfig::init_logging()
{
    ...
    if (m_settings) {
        configuredLevel = m_settings->value(KEY_LOG_LEVEL, DEFAULT_LOG_LEVEL).toString();
    }
    ...
    spdlog::info("Logging initialized with level: {}", configuredLevel.toStdString());
}

void QGCSConfig::qtLogHandler(QtMsgType type,
    ...
    const QString& msg)
{
    ...
    switch (type) {
    case QtDebugMsg:
        spdlog::debug("[QT] {}", local.constData());
        break;
    case QtInfoMsg:
        spdlog::info("[QT] {}", local.constData());
        break;
    case QtWarningMsg:
        spdlog::warn("[QT] {}", local.constData());
        break;
    case QtCriticalMsg:
    case QtFatalMsg:
        spdlog::error("[QT] {}", local.constData());
        break;
    }
}
...
{
    if (!m_settings) return QString(DEFAULT_LOG_LEVEL);
